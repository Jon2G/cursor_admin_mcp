...
describe("runAsRoot", () => {
  it("returns error on non-macOS platforms", async () => {
    ...
    expect(result.content[0]?.text).toContain("only supported on macOS");
  });
  ...
  it("writes payload, wrapper, and askpass files, then returns output", async () => {
    ...
    expect(writtenFiles.get(paths.wrapperFile)).toContain("osascript");
    expect(writtenFiles.get(paths.askpassFile)).toContain("hidden answer");
  ...
  it("returns user denial message without error flag", async () => {
    ...
    const result = await runAsRoot("softwareupdate -i -a", {
      ...baseDeps(),
